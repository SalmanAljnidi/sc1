...
function renderStandbyReports(afterSchedule=false){
  ...
  const matrix = `
    ...
    <div class="matrix-wrap"><table class="standby-matrix">
...
function buildSchedule(){
  ...
  function scoreSlot(grid, day, period, lesson){
    let sc=0;
    const subj=lesson.subject;
    // avoid always same period for same subject داخل نفس الفصل
    let same=0;
    for(let d=0;d<5;d++){
    ...
    // توازن بسيط: لا نُفضِّل الحصة الأولى بشكل زائد
    // تفضيل خفيف للحصص الوسط (3-5) لتقليل "كل المواد في الأولى"
    const mid = 3; // (0..6)
    sc -= Math.abs(period-mid)*0.06;

    // تفضيل خفيف لتوزيع المواد عبر أيام الأسبوع (تجنب حشد نفس المادة في يوم واحد إن أمكن)
    let sameDay=0;
    for(let p=0;p<7;p++){
      const c = grid[day][p];
      if(c && c.subject===subj) sameDay++;
    }
    sc -= sameDay*0.8;

    // ضوضاء صغيرة لتجنب الانحياز الثابت
    sc += (Math.random()*0.01);
    return sc;
  }
  ...
  function bestOf(attempts, relaxRules){
    ...
    const quality = (run)=>{
      // 1) عدد الحصص المتعذرة
      const u = run.unscheduled.length;

      // 2) عدد الخانات الفارغة داخل أيام الدوام (داخل lens)
      let blanks=0;
      for(const s of Object.values(run.sectionSchedules)){
        for(let d=0; d<5; d++){
          for(let p=0; p<7; p++){
            if(p>=s.lens[d]) continue;
            if(s.grid[d][p]==="") blanks++;
          }
        }
      }

      // 3) انحياز للحصة الأولى/الأخيرة (كلما قل كان أفضل)
      let skew=0;
      for(const s of Object.values(run.sectionSchedules)){
        for(let d=0; d<5; d++){
          const c0 = (0<s.lens[d]) ? s.grid[d][0] : null;
          const c6 = (6<s.lens[d]) ? s.grid[d][6] : null;
          if(c0 && c0!=="") skew += 1;
          if(c6 && c6!=="") skew += 0.6;
        }
      }

      // ترتيب lexicographic: u ثم blanks ثم skew
      return {u, blanks, skew};
    };
    ...
    for(let i=0;i<attempts;i++){
      ...
      const q = quality(run);
      if(!best || better(q,bestScore)){
        best = run;
        bestScore = q;
        if(bestScore.u===0 && bestScore.blanks===0) break;
      }
    }
    return best;
  }
  ...
  // المرحلة 1: صارمة
  const strictRun = bestOf(220, false);
  // المرحلة 2: (اختياري) تخفيف قيود لتقليل الفراغات
  const relaxedRun = forceFill ? bestOf(420, true) : null;
  ...
  for(const t of state.teachers){
    ...
    // توزيع الانتظار على أيام متفرقة قدر الإمكان (مع توازن على الحصص)
    const baseDays=[0,1,2,3,4];
    const basePeriods=[4,3,5,2,6,1,0]; // ليست آخر اليوم فقط
    const hashId = (s)=>{ let h=0; for(let i=0;i<String(s).length;i++){ h=(h*31 + String(s).charCodeAt(i))>>>0; } return h; };
    const dayShift = hashId(t.id)%5;
    const perShift = hashId(t.id)%7;
    const dayOrder = baseDays.slice(dayShift).concat(baseDays.slice(0,dayShift));
    const periodOrder = basePeriods.slice(perShift).concat(basePeriods.slice(0,perShift));
    // جهّز مرشحين لكل يوم
...
function shortSectionLabel(sectionKey){
  const [sid,grade,sec] = sectionKey.split(":");
  // اختصار مفيد لعرض جداول الشامل
  return `${gradeLabel(sid,grade)}/${sec}`;
}

function fullSectionLabel(sectionKey){
  const s = state.schedule?.sections?.[sectionKey];
  if(!s) return sectionKey;
  // meta.label usually already full (مرحلة/صف/فصل)
  return s.meta?.label || sectionKey;
}


function renderSectionsMatrix(){
  const lens = state.schedule.globalLens || [7,7,7,7,7];
  const secs = Object.values(state.schedule.sections);
  if(!secs.length) return "";

  const head1 = `<tr><th rowspan="2" class="teacher-name">الفصل</th>` +
    DAYS.map(d=>`<th class="day" colspan="7">${d}</th>`).join("") + `</tr>`;
  const head2 = `<tr>` + DAYS.flatMap(()=>Array.from({length:7},(_,i)=>`<th>${i+1}</th>`)).join("") + `</tr>`;

  const body = secs.map(s=>{
    const tds=[];
    for(let di=0; di<5; di++){
      for(let pi=0; pi<7; pi++){
        if(pi>=s.lens[di]) { tds.push(`<td class="off"></td>`); continue; }
        const cell = s.grid[di][pi];
        if(cell==="") { tds.push(`<td></td>`); continue; }
        const subj = cell.subject || "";
        tds.push(`<td><div class="cell-main" title="${escapeHtml(subj)}">${escapeHtml(subj)}</div></td>`);
      }
    }
    return `<tr><th class="teacher-name">${escapeHtml(s.meta.label)}</th>${tds.join("")}</tr>`;
  }).join("");

  return `<div class="schedule-block">
    <h3>جدول شامل للفصول (في جدول واحد)</h3>
    <div class="matrix-wrap"><table class="wide-matrix">
      <thead>${head1}${head2}</thead>
      <tbody>${body}</tbody>
    </table></div>
  </div>`;
...
function renderTeachersMatrix(){
  const lens = state.schedule.globalLens || [7,7,7,7,7];
  const ts = Object.values(state.schedule.teachers);
  if(!ts.length) return "";

  const head1 = `<tr><th rowspan="2" class="teacher-name">المعلم</th>` +
    DAYS.map(d=>`<th class="day" colspan="7">${d}</th>`).join("") + `</tr>`;
  const head2 = `<tr>` + DAYS.flatMap(()=>Array.from({length:7},(_,i)=>`<th>${i+1}</th>`)).join("") + `</tr>`;
  ...
  const body = ts.map(t=>{
    ...
    for(let di=0; di<5; di++){
      for(let pi=0; pi<7; pi++){
        if(pi>=lens[di]) { tds.push(`<td class="off"></td>`); continue; }
        const v = t.grid[di][pi];
        if(v==="") { tds.push(`<td></td>`); continue; }
        if(v===SUBJECTS.STANDBY) { tds.push(`<td class="standby">${SUBJECTS.STANDBY}</td>`); continue; }
        // v هنا sectionKey
        tds.push(`<td><div class="cell-main" title="${escapeHtml(fullSectionLabel(v))}">${escapeHtml(shortSectionLabel(v))}</div></td>`);
      }
    }
    return `<tr><th class="teacher-name">${escapeHtml(t.teacher.name)}</th>${tds.join("")}</tr>`;
  }).join("");

  return `<div class="schedule-block">
    <h3>جدول شامل للمعلمين (في جدول واحد)</h3>
    <div class="matrix-wrap"><table class="wide-matrix">
      <thead>${head1}${head2}</thead>
      <tbody>${body}</tbody>
    </table></div>
...
function renderAllSections(){
  ...
  if(!state.schedule){ wrap.innerHTML=`<div class="hint">أنشئ الجدول أولاً.</div>`; return; }
  const secs=Object.values(state.schedule.sections);
  const matrix = renderSectionsMatrix();
  const cards = secs.map(s=>`<div class="schedule-block">${renderSectionTable(s)}</div>`).join("");
  wrap.innerHTML = matrix + cards;
}
...
function renderStandbyAll(){
  ...
  const rows = ts.map(t=>{
    ...
    return {id:t.teacher.id, name:t.teacher.name, spec:t.teacher.spec, max:t.teacher.max,
      assigned, wanted, placed:count, remaining:Math.max(0,wanted-count)};
  }).sort((a,b)=> (b.placed-b.remaining) - (a.placed-a.remaining));
  ...
  const summary = `
    <div class="schedule-block">
      ...
      <table>
        <thead><tr>
          <th>المعلم</th><th>التخصص</th><th>النصاب</th><th>المسند</th>
          <th>الانتظار المطلوب</th><th>الانتظار المجدول</th><th>المتبقي</th>
        </tr></thead>
        <tbody>
          ${rows.map(r=>`
            <tr>
              <td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.spec)}</td><td>${r.max}</td><td>${r.assigned}</td>
              <td>${r.wanted}</td><td>${r.placed}</td><td>${r.remaining}</td>
            </tr>`).join("")}
        </tbody>
      </table>
      <div class="hint">ملاحظة: لا يتم السماح بأكثر من حصتين انتظار في نفس الحصة على مستوى المدرسة.</div>
    </div>`;

  // big matrix like الفصول/المعلمين
  const head1 = `<tr><th rowspan="2" class="teacher-name">المعلم</th>` +
    DAYS.map(d=>`<th class="day" colspan="7">${d}</th>`).join("") + `</tr>`;
  const head2 = `<tr>` + DAYS.flatMap(()=>Array.from({length:7},(_,i)=>`<th>${i+1}</th>`)).join("") + `</tr>`;
  const body = ts.map(t=>{
    ...
    for(let di=0; di<5; di++){
      for(let pi=0; pi<7; pi++){
        if(pi>=lens[di]) { tds.push(`<td class="off"></td>`); continue; }
        const v = t.grid[di][pi];
        if(v===SUBJECTS.STANDBY) tds.push(`<td class="standby">${SUBJECTS.STANDBY}</td>`);
        else tds.push(`<td></td>`);
      }
    }
    return `<tr><th class="teacher-name">${escapeHtml(name)}</th>${tds.join("")}</tr>`;
  }).join("");

  const matrix = `
    <div class="schedule-block">
      <h3>جدول الانتظار الأسبوعي (شامل)</h3>
      <div class="matrix-wrap"><table class="wide-matrix standby-matrix">
        <thead>${head1}${head2}</thead>
        <tbody>${body}</tbody>
      </table></div>
    </div>`;
...
function renderAllTeachers(){
  ...
  if(!state.schedule){ wrap.innerHTML=`<div class="hint">أنشئ الجدول أولاً.</div>`; return; }
  const ts=Object.values(state.schedule.teachers);
  const matrix = renderTeachersMatrix();
  const cards = ts.map(t=>`<div class="schedule-block">${renderTeacherTable(t)}</div>`).join("");
  wrap.innerHTML = matrix + cards;
}
