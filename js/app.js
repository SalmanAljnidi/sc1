  ...
  const body = secs.map(s=>{
    ...
    for(let di=0; di<5; di++){
      for(let pi=0; pi<7; pi++){
        ...
        tds.push(`<td><div class="cell-main">${escapeHtml(subj)}</div></td>`);
      }
    }
  ...
  const body = ts.map(t=>{
    ...
    for(let di=0; di<5; di++){
      for(let pi=0; pi<7; pi++){
        ...
        // v هنا sectionKey
        tds.push(`<td><div class="cell-main">${escapeHtml(shortSectionLabel(v))}</div></td>`);
      }
    }
...
function renderStandbyAll(){
  ...
  const matrix = `
    <div class="schedule-block">
      ...
      <table class="wide-matrix standby-matrix">
        <thead>${head1}${head2}</thead>
        <tbody>${body}</tbody>
      </table>
    </div>`;
